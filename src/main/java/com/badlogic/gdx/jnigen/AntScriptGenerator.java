...
public class AntScriptGenerator {
	...
	public void generate (BuildConfig config, com.badlogic.gdx.jnigen.BuildTarget... targets) {
		...
		for (com.badlogic.gdx.jnigen.BuildTarget target : targets) {
			...
			if (target.os == TargetOs.IOS) {
				byte[] plist = new com.badlogic.gdx.jnigen.FileDescriptor("com/badlogic/gdx/jnigen/resources/scripts/Info.plist.template", FileType.Classpath)
						.readBytes();
	...
	private void copyJniHeaders (String jniDir) {
		...
		for (String file : files) {
			new com.badlogic.gdx.jnigen.FileDescriptor(pack, FileType.Classpath).child(file).copyTo(
	...
	public static String getLibsDirectory (BuildConfig config, com.badlogic.gdx.jnigen.BuildTarget target) {
	...
	private String generateBuildTargetTemplate (BuildConfig config, BuildTarget target) {
		...
		if (target.os == TargetOs.Android) {
			...
			String template = new com.badlogic.gdx.jnigen.FileDescriptor("com/badlogic/gdx/jnigen/resources/scripts/build-android.xml.template",
